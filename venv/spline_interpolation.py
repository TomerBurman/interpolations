...
def natural_spline(table,x):
    ...
    print(m_list)
    i = 0
    if x < table[0][0]: # extrapolation when input is smaller than first point.
...
def calc_d(h_list,table, f_tagZero = 0,f_tagN = 0):
    ...
    for i in range(1,len(h_list)): # table is larger than h_list, we dont need -1
        d_list.append((6 /(h_list[i] + h_list[i-1])) * (((table[i+1][1] - table[i][1]) / h_list[i]) - ((table[i][1] - table[i-1][1]) / h_list[i-1])))
...
natural_spline([(0,0),(math.pi / 6, 0.5), (math.pi / 4, 0.7072), (math.pi / 2, 1 )],math.pi / 3)